"""Compare the baseline good-shape model with Hwang et al. (2024).

This is a branch-only mechanism diagnostic.  It keeps the same three observed
systems and evaluates their Rice-convolved good-branch likelihood while
varying trial total mass.  It does not build a lookup or refit the MLR.
"""
...
SHAPES = {
    ...
    "Hwang et al. (2024)": (0.00224, 36.09, 3.85),
}


def normalized_good_density(x: np.ndarray, shape: tuple[float, float, float]) -> tuple[np.ndarray, float]:
    """Return the normalized good density and its raw basis integral."""
    B, uc, C = shape
    A = hm.RICE_GOOD_A
...
def choose_examples(u: np.ndarray, u_sigma: np.ndarray) -> list[int]:
    """Choose the same representative systems as the earlier shape figure."""
    chosen: list[int] = []
...
def main() -> None:
    ...
    parser.add_argument(
        "--lookup",
        type=Path,
        default=ROOT / "results/t8_raw_u_solar_only_fixed_good_20260924/dynamics_likelihood_lookup_t8.npz",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results/t8_good_shape_hwang_comparison_20260924",
    )
            ...
            values[i] = [
                np.log(
                    hm.rice_component_reference_integral(
                        float(u[row]), float(u_sigma[row]), np.sqrt(m), component="good"
                    )
                )
                for m in mass
            ]
    ...
    fig, (ax0, ax1) = plt.subplots(
        2, 1, figsize=(9.2, 7.2), gridspec_kw={"height_ratios": [1.0, 1.35]}
    )
    colors = ["#3568a8", "#4c9f70", "#c44e52"]
    ax0.plot(
        x,
        density["Baseline"],
        color="#3568a8",
        lw=2.2,
        label=r"Baseline: $B=0.002544$, $u_c=35.67$, $C=3.10$",
    )
    ax0.plot(
        x,
        density["Hwang et al. (2024)"],
        color="#d27a28",
        lw=2.2,
        ls="--",
        label=r"Hwang et al. (2024): $B=0.00224$, $u_c=36.09$, $C=3.85$",
    )
    ax0.set_ylabel(r"Normalized $p_g(\tilde u)$")
    ax0.set_xlabel(r"Scaled velocity $\tilde u$ (km s$^{-1}$ au$^{1/2}$)")
    ax0.set_title(r"Baseline and Hwang et al. (2024) good-component shapes")
    ...
    for i, (row, color) in enumerate(zip(chosen, colors)):
        ax1.plot(
            ...
            color=color,
            lw=2.0,
            label=f"$u={u[row]:.1f}$, $\\sigma_u={u_sigma[row]:.1f}$",
        )
        ax1.plot(mass, delta["Hwang et al. (2024)"][i], color=color, lw=1.8, ls="--")
    ...
    ax1.axvline(
        ...
        lw=1.2,
        ls=":",
        label=r"Reference mass $M_{\rm tot}=1.00\,M_\odot$",
    )
    ...
    ax1.text(
        0.99,
        0.03,
        "Solid = baseline; dashed = Hwang et al. (2024)",
    ...
    fig.savefig(args.output_dir / "good_shape_hwang_comparison.png", dpi=240, bbox_inches="tight")
    fig.savefig(args.output_dir / "good_shape_hwang_comparison.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    summary = {
        ...
        "lookup": str(args.lookup.resolve()),
        "n_valid_systems": int(u.size),
        "support_max": 80.0,
        "reference_mass_msun": 1.0,
        "shapes": {
            name: {"B": shape[0], "uc": shape[1], "C": shape[2], "raw_basis_integral": raw_norm[name]}
            for name, shape in SHAPES.items()
        },
        "representative_systems": records,
        "delta_logL_at_1p10": {
            name: [float(values[i, np.argmin(np.abs(mass - 1.10))]) for i in range(len(chosen))]
            for name, values in delta.items()
        },
        "interpretation": "Good-component branch diagnostic only; no mixture fraction or MLR refit.",
        "selection_reason": "Same fixed T8 systems as the earlier mechanism figure: nearest valid observed u to 10, 20, and 30 with ordinary u_sigma near 2.",
        "literature_note": "The requested Hwang et al. comparison is labeled 2024, matching the cited MNRAS paper and repository context.",
    }
