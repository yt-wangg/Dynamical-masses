...
\citep{LiuBayarriBerger2009Modularization,Plummer2015Cuts}.  Stage~1 combines the
two component-level JCAPS metallicity measurements with the PARSEC
colour--magnitude diagram (CMD) to infer a probability distribution for the
shared metallicity of every binary.  Stage~2 treats these distributions as
...
\node[model]  (stage1) at (4.1, 1.8) {Stage 1: shared $Z_j$ + \mbox{PARSEC} CMD\\+ population prior};
...
\end{tikzpicture}
\caption{Scientific workflow.  The first stage returns a posterior probability
mass over the shared system metallicity.  The second stage propagates this
uncertainty into the dynamical MLR fit.  The red cut boundary is a modeling
choice: the dashed red label emphasizes that the lower-stage data do not update
the upper-stage calibration.}
\label{fig:workflow}
\end{figure*}
...
\midrule
$\mathcal D_{\rm 1}$ & Stage-1 JCAPS and Gaia CMD data used for metallicity calibration & data set \\
$\mathcal D_{\rm 2}$ & Stage-2 component-magnitude and wide-binary dynamical data, with $\overline P_{jq}$ supplied from Stage~1 & data set \\
...
$\overline P_{jq}$ & Stage-1 posterior probability mass at $Z_q$ & $[0,1]$ \\
...
\section{Stage 1: shared system metallicities}
\label{sec:stage1}

\subsection{Relative JCAPS calibration}
...
$\nu$.  The inherited calibration maximized
\begin{equation}
 \widehat{\bm\theta}
 =\arg\max_{\bm\theta}\sum_{j\in\mathcal D_{\rm 1}}
 \log p(d_j\mid\bm\theta),
 \qquad \bm\theta=(b_G,s_z,\nu),
\end{equation}
and checked the same difference distribution on a held-out subset.  They
cannot determine an absolute metallicity zero point.  We set
...
\end{align}
with linear interpolation between knots.  These are fixed inputs in Stage~1,
not quantities re-estimated here.  The remaining common offset is assigned
\begin{equation}
...
% \subsection{Population distribution of true metallicity}

% The system metallicities are drawn from a flexible density
% \begin{equation}
...
% $-1.0\le Z\le0.6$ dex.  The six centers are equally spaced from $-1.0$ to
% $0.6$ dex and all component scales are $1.6/6=0.2667$ dex.  This is a prior for
% the latent true distribution of the selected systems, not an empirical density
% formed by treating $\widehat z$ as error-free.

% Writing $\phi$ and $\Phi$ for the standard-normal density and cumulative
% distribution function, respectively, each normalized basis is
% \begin{equation}
%  \varphi_r^{\rm trunc}(Z)=
...
% \end{equation}
% with $Z_{\min}=-1.0$ and $Z_{\max}=0.6$ dex.

\subsection{PARSEC CMD likelihood and the 4000-K mask}
...
\begin{equation}
 C_{jk} \mid Z_j \sim \StudentT_{\nu}\!\left[
 C_{\rm P}(x_{jk},Z_j),
 \sqrt{\sigma_{C,jk}^2+s_C^2}\right] =  \mathcal{L}^{\rm CMD}_{jk}(Z_j).
 %\qquad s_C\sim\operatorname{HalfNormal}(0.1\ {\rm mag}).
 \label{eq:cmd}
\end{equation}
The colour zero point is fixed to zero.  To avoid allowing membership in the
...
\subsection{System posterior and the cut object}

For global Stage-1 parameters
$\bm\psi=(\delta_z,s_C,b_G,s_z)$, the unnormalized posterior density for one
system is
\begin{equation}
 \widetilde p_j(Z\mid\bm\psi)=p_{\rm pop}(Z) \cdot
 \prod_{k=1}^{2}\mathcal{L}^{\rm spec}_{jk}(Z\mid\bm\psi) \cdot 
 \left[\mathcal{L}^{\rm CMD}_{jk}(Z\mid\bm\psi)\right]^{m_{jk}}.
 \label{eq:zpost-cont}
\end{equation}
...
\begin{equation}
 p(\bm\psi\mid\mathcal D_{\rm 1})\propto
 p(\delta_z)\,p(s_C)\,p(b_G)\,p(s_z)
 \prod_j\left[\sum_{q\in\mathcal Q}w_q^{(Z)}
 \widetilde p_j(Z_q\mid\bm\psi)\right].
...
\begin{equation}
 P_{jq}(\bm\psi)=
 \frac{w_q^{(Z)}\widetilde p_j(Z_q\mid\bm\psi)}
 {\sum_{q'}w_{q'}^{(Z)}\widetilde p_j(Z_{q'}\mid\bm\psi)}.
\end{equation}
The saved quantity passed to Stage~2 is the posterior average
\begin{equation}
 \overline P_{jq}=\mathbb E_{\bm\psi\mid\mathcal D_{\rm 1}}
 \left[P_{jq}(\bm\psi)\right].
 \label{eq:pbar}
\end{equation}
Thus Stage~2 receives a probability distribution rather than a metallicity
point estimate.  Because $\overline P_{jq}$ already contains the population
prior, both spectroscopic terms, eligible CMD terms, and metallicity integration
weights, none of these factors is multiplied in again below.
...
\begin{equation}
    u := v \sqrt{s},
\end{equation}
where $v$ is the projected relative speed and $s$ is the projected separation. Under this definition, $u \propto \sqrt{m_{\rm tot}}$ and depends on nuisance parameters $\theta$, $e$, and orientation $\phi$. We again separate the mass dependence by defining $\tilde{u} \equiv u/\sqrt{m_{\rm tot}}$, so $\tilde{u}$ depends only on the nuisance parameters and their influence can be marginalized. We perform simulations and pre-fit $p(\tilde{u})$ by integrating $p(\tilde{u}\,|\,\phi,\theta,e)$ over the nuisance distributions. For an observed $u$ the likelihood then becomes
\begin{equation}
    p(u\,|\,m_{\rm tot}) = \frac{1}{\sqrt{m_{\rm tot}}}\,p\!\left(\tilde{u} \equiv \frac{u}{\sqrt{m_{\rm tot}}}\right).
\end{equation}
...
\end{equation}
with parameters $\Theta$ defining the functional form. The likelihood for one measurement is then

\begin{align}
    &\mathcal{L}_j \\
    &\quad = p \left(u_j \big|\ m_{\rm tot};\mathcal{M}(m_{j,1}, Z_{j}) + \mathcal{M}(m_{j,2},Z_{j});\Theta \right). \notag
\end{align}
...
\begin{align}
    &p(u_j\,|\,m_{\rm tot}) \\
    &= \frac{1}{\sqrt{m_{\rm tot}}} \int_0^\infty \mathrm{Rice}\!\left(\frac{u_j}{\sqrt{m_{\rm tot}}} \,\bigg|\, \tilde{u}, \frac{\sigma_{u,j}}{\sqrt{m_{\rm tot}}}\right) p(\tilde{u})\, d\tilde{u}. \notag
\end{align}
...
\begin{equation}
\begin{aligned}
    \mathcal{L}^{\rm mix}_j = \int~ \Biggl[ & (1-f_{\mathrm{outlier}}) \cdot \dfrac{1}{\sqrt{m_{\mathrm{tot}}}} \cdot p\left( \tilde{u} \right) \\
    & + f_{\mathrm{outlier}} \cdot \dfrac{1}{\sqrt{m_{\mathrm{tot}}}} \cdot \dfrac{1}{C_{\mathrm{o}}} \cdot G_{\mathrm{o}}\left( \tilde{u}|\mu_{\mathrm{o}},~\sigma^2_{\mathrm{o}} \right) \Biggr] \\
    & \times \mathrm{Rice} \left(\dfrac{u_{\mathrm{obs}}}{\sqrt{m_{\mathrm{tot}}}}|\tilde{u},\dfrac{\sigma_{u,j}}{\sqrt{m_{\mathrm{tot}}}}\right) ~d\tilde{u}.
\end{aligned}
\end{equation}
...
\begin{align}
    m_{\mathrm{tot}} = \mathcal{M}(M_{\mathrm{G},1}) + \mathcal{M}(M_{\mathrm{G},2})
\end{align}
...
% \begin{equation}
%  L_j^{\rm mix}(s)=(1-f_{\rm out})L_{{\rm good},j}(s)
%  +f_{\rm out}L_{{\rm out},j}(s),
%  \qquad f_{\rm out}\sim\operatorname{Beta}(3,12).
%  \label{eq:mixture}
% \end{equation}
% Because \cref{eq:good} retains its raw, slightly non-normalized amplitude,
% $f_{\rm out}$ is an effective likelihood weight and cannot be interpreted as
...
\begin{equation}
 ...
 \sum_{q=1}^{81}\overline P_{jq}\, \cdot
 \mathcal L_j^{\rm mix}\!\left[s_{jq}(\bm\theta_M)\right].
 \label{eq:dyn-final}
\end{equation}
This sum propagates the system's Stage-1 metallicity uncertainty into its mass
and dynamics.  It is not a second use of the spectroscopic or CMD data, and no
additional quadrature weight belongs in \cref{eq:dyn-final} because it is
already present in the probability masses $\overline P_{jq}$.
...
\begin{table}[htbp]
\centering
\caption{Frozen Stage-1 and Stage-2 parameter specification.  Normal
...
\midrule
1 & $\delta_z$ & $\Normal(0,0.3)$ dex & JCAPS--PARSEC zero point \\
1 & $s_C$ & Half-Normal$(0.1)$ mag & CMD excess colour scatter \\
1 & $\bm\omega$ & Dirichlet$(1,\ldots,1)$ & Six-component population weights \\
1 & $b_G,s_z,\nu$ & fixed T6d values & Relative JCAPS calibration \\
\addlinespace
...
2 & $f_{\rm out}$ & Beta$(3,12)$ & Effective component weight \\
...
Both stages were sampled with the No-U-Turn Sampler (NUTS)
...
distribution, or T6d plug-in calibration.
...
& Uncertainty in fixed $b_G$, $s_z$, and $\nu$; cross-system covariance
induced by shared Stage-1 parameters \\
...
Feedback & Metallicity uncertainty is passed from Stage~1 to Stage~2
...
model, but it understates joint uncertainty by discarding the shared Stage-1
posterior covariance and any information that the dynamics could provide about
metallicity calibration.  A fully joint analysis would be a different model,
