...
\midrule
$\mathcal D_{\rm 1a}$ & Within-binary JCAPS differences used to estimate relative calibration parameters & data set \\
...
on $Z_j$, the two component measurements are independent.


\subsection{Stage 1b: Absolute calibration and system metallicity posteriors}

\subsubsection{CMD likelihood}

In this section, we can make use of any set of anchor points whose metallicties are robust and cover a wide enough range. Here's an example when we directly use the PARSEC model points of warm main-sequence stars.  The PARSEC surface supplies predicted colour $C_{\rm P}(x,Z)$ and effective temperature $T_{\rm eff,P}(x,Z)$ \citep{Bressan2012PARSEC}.  The repository
...
The sampled Stage-1a and Stage-1b global parameters are 
$\bm\psi=(b_{\mathrm{G}},s_z,\delta_z,s_C)$.  The unnormalized metallicity density for one system is
\begin{equation}
 \widetilde p_j(Z,\bm\psi \mid \mathcal{D})
 =p_{\rm pop}(Z) p(\psi)
 \prod_{k=1}^{2}\mathcal{L}^{\rm spec}_{jk}
 (D_{\rm 1a}\mid Z,\bm\psi)
 \left[\mathcal{L}^{\rm CMD}_{jk}(D_{\rm 1b}\mid \bm\psi)\right]^{m_{jk}}.
 \label{eq:zpost-cont}
\end{equation}
The global parameters are inferred from the product of the system-level marginal evidences, rather than from separately normalized system posteriors.
...
\begin{equation}
\begin{aligned}
\mathcal{L}^{\mathrm{mix}}_j
= \int \Biggl[
& (1-f_{\mathrm{outlier}}) \cdot \frac{p(\tilde{u})}{\sqrt{m_{\mathrm{tot}}/M_{\odot}}} \\
& + f_{\mathrm{outlier}} \cdot
\frac{G_{\mathrm{o}}(\sqrt{m_{\mathrm{tot}}} \cdot \tilde{u}\mid\mu_{\mathrm{o}},\sigma_{\mathrm{o}}^2)}
{C_{\mathrm{o}}}
\Biggr] \\
& \times \mathrm{Rice}\left(
\frac{u_{\mathrm{obs}}}{\sqrt{m_{\mathrm{tot}}/M_{\odot}}}
\mid \tilde{u},
\frac{\sigma_{u,j}}{\sqrt{m_{\mathrm{tot}}/M_{\odot}}}
\right)\,d\tilde{u},
\end{aligned}
\end{equation}
where the outlier fraction $f_{\mathrm{outlier}}$ is a free parameter, and $G_{\mathrm{o}}$ is a Gaussian distribution with mean $\mu_{\mathrm{o}}$ and variance $\sigma_{\mathrm{o}}^2$ (defined on velocity scale $u$). The normalization constant $C_{\mathrm{o}}$ ensures that the outlier distribution integrates to unity over the relevant range of $\tilde{u}$.
